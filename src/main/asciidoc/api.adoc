...
Defines an unique ID to be referenced somewhere else. Also declares the class name as a `class` attribute.
...
Note: you can find description of each property http://gdrouet.github.io/wuic/apidocs/constant-values.html#com.github.wuic.ApplicationConfig[here]
...
If you want to provide parameters in a different way, specify an implementation of `com.github.wuic.util.BiFunction` (inspired from Java 8).
It must be parameterized with String type only.
The first parameter of `apply()` expects the key and the second parameter the default value.
...
<table>
    ...
    <tr>
        ...
        <td>
            List of comma-separated class names extending http://wuic.github.io/apidocs/index.html?com/github/wuic/ContextBuilderConfigurator.html[ContextBuilderConfigurator]
    ...
    <tr>
        ...
        <td>
            List of comma-separated class names extending http://wuic.github.io/apidocs/index.html?com/github/wuic/config/ObjectBuilderInspector.html[ObjectBuilderInspector]
            that will be used in the facade.
        </td>
        <td>Empty string</td>
    </tr>
    ...
    <tr>
         ...
         <td>Indicates the com.github.wuic.util.BiFunction implementation which retrieves a value for a given parameter. Of course, default implementation relies on init-param in web.xml file.</td>
...
You should enable the best effort mode: if the HTML has been parsed before and the result still exists in the cache, it applies replacements.
However, it just return the unparsed HTML to deliver as fast as possible the response and process scripts asynchronously.
TODO: migrate best effort and include filter section about that
...
This means that if two page lead to the same ID, you will start facing weird issues.
