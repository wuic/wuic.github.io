...
Some components previously mentioned are internally using temporary files for different purpose.
For instance:

* if a `NutDao` is resolving a file inside a `zip` file, it needs to extract it somewhere.
* the built-in `Engine` that caches resources of the disk also needs to serialize the object somewhere.

WUIC uses its dedicated temporary directory where it creates and deletes the files later.

By default, files are created from a directory called `wuic` located inside the system temporary directory corresponding to the `java.io.tmpdir` system property.
Moreover, files are deleted 50 minutes after their creation or when the JVM shutdowns properly.
The time to live can also be specific for the component that created it.
For instance, built-in cache uses it's own time to live value.

You can change the location of temporary directory used by WUIC thanks to the `com.github.wuic.tmpDir` property.
Time to live of temporary files can also be changed via the `com.github.wuic.tmpFileTimeToLiveSeconds` property.
...
<table>
    ...
    <tr>
         <td>*.facade.maxWorkflowExecutionStats</td>
         <td>n last execution statistics retained for each workflow.</td>
         <td>1000</td>
    </tr>
    <tr>
         <td>*.facade.maxHeapResolutionStats</td>
         <td>n last resolution statistics retained for each heap.</td>
         <td>1000</td>
    </tr>
    <tr>
         <td>*.charset</td>
...
=== data-wuic-break: split bundle

If a sequence of assets of the same type is detected in a HTML page, WUIC will aggregate them.
In some case, it could be preferred to create more than one bundle in order to take advantage from caching capabilities.
For instance, most of the time you will prefer to put all libraries not subject to change in one isolated file.

To achieve this, you can use the `data-wuic-break` attribute in the asset's tag.
This directive will stop the current sequence of assets and will start to accumulate the new detected assets in a separate bundle.
...
<script data-wuic-break src="script2.js"></script>
<script src="script3.js"></script>
<script src="script4.js"></script>
----

WUIC will replace the HTML code above with two `<script>` tags, the first one containing `script1.js` and the second one a bundle containing the three other files.

NOTE: if you don't want script generated with JSP tag to be aggregated with the other scripts, you can add to your `wuic:html-import` tag a `breakAggregation` attribute.
With the Thymeleaf processor, you can add the `data-wuic-break` attribute to the HTML element declared with the `wuic:import` attribute.
