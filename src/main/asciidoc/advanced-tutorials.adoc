...
* `@Config` annotation for you constructor / initialization method
...
INFORMATION: if you provide a default constructor that is not annotated with `@Config`, WUIC will use it if not other valid
constructor correctly annotated has been found. Also note that any method correctly annotated will be executed after the instantiation.
