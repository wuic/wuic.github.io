...
IMPORTANT: by default the charset is an empty string.
In that case, WUIC will use `Charset.defaultCharset()` to retrieve a valid value.
...
Another solution is to define the system property `file.encoding` in the JVM settings of each environment where WUIC is running.
